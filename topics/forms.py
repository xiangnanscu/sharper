...
TOPIC_CONTENT_MAX_LENGTH = getattr(settings,'TOPIC_CONTENT_MAX_LENGTH', 2000)
TOPIC_CONTENT_MIN_LENGTH = getattr(settings,'TOPIC_CONTENT_MIN_LENGTH', 3)
TOPIC_TITLE_MAX_LENGTH = getattr(settings,'TOPIC_TITLE_MAX_LENGTH', 30)
TOPIC_TITLE_MIN_LENGTH = getattr(settings,'TOPIC_TITLE_MIN_LENGTH', 3)
...
class TopicForm(forms.Form):
    ...
    content=forms.CharField(
        label="",
        widget=forms.Textarea(attrs={
            'placeholder':'点击这里输入帖子内容',
            'rows':20,
            'cols': 80,
        }),
        max_length=TOPIC_CONTENT_MAX_LENGTH,
    ) 
    tags=forms.CharField(
        label="",
        widget=forms.Textarea(attrs={
            'placeholder':'为该帖子选择1到%s个标签,多个标签用逗号或分号分隔'%TAG_MAX_LENGTH,
            'rows':1,
            'cols': 80,
        }),
    ) 
    ...
    def clean_title(self):
        ...
        if not TOPIC_TITLE_MIN_LENGTH<=len(title)<=TOPIC_TITLE_MAX_LENGTH:
    ...
    def clean_content(self):
        ...
        if not TOPIC_CONTENT_MIN_LENGTH<=len(content)<=TOPIC_CONTENT_MAX_LENGTH:
            raise forms.ValidationError('字符数不合法')
        if settings.COMMENTS_ALLOW_PROFANITIES == False:
    ...
    def create(self):
        return Topic(**self.get_create_data())
...
class CaptchaTopicForm(TopicForm):
    captcha = CaptchaField(label='')